...
res.send("<script>alert('Invalid Username or Password');window.location.href='/admin-login.html';</script>");
});
...
```
res.send("<script>alert('Job Posted Successfully');window.location.href='/admin-dashboard.html';</script>");
```
...
const sql = 'INSERT INTO applications (job_id, full_name, mobile, age, dob, gender, marital_status, permanent_address, current_address, photo, aadhaar, pan_card, resume) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
const sql = 'SELECT applications.*, jobs.title AS job_title FROM applications LEFT JOIN jobs ON applications.job_id = jobs.id ORDER BY applications.created_at DESC';
